...
// Desafio lógico
let athleteEnergy: number = 5;
let numberPushUpsLap: number = 3;
let lap: number = 0;

while (athleteEnergy > 0) {
  console.log(`Energia: ${athleteEnergy}`);
  console.log(`Atleta correndo uma volta`);

  for (let i = 0; i < numberPushUpsLap || athleteEnergy == 0; i++) {
  ...
  if (athleteEnergy >= 0) {
    console.log(`O atleta completou a volta ${lap}`);
  } else {
    console.log(`O atleta não completou a volta ${lap}`);
  }
